    ...
    public boolean onCommand(CommandSender s, Command cmd, String label, String[] args) {
        if(s instanceof Player){
            ...
            if(p.hasPermission("litebans.history")){
                if (args.length == 0) {
                    p.sendMessage(Utils.chat("&b&lGlowMC &7>> &c/history <player>"));
                } else if (args.length == 1) {
                    boolean checker = true;
                    for (OfflinePlayer player : Bukkit.getServer().getOfflinePlayers()) {
                        if (player.getName().equalsIgnoreCase(args[0])) {
                            checker = false;
                            GUIManager.HistoryGUI(player.getUniqueId(), p);
                        }
                    }
                    if(checker){
                ...
                p.sendMessage(Utils.chat("&cYou dont have permission to perform this command!"));
            }
        }else{
            s.sendMessage(Utils.chat("&c[ERROR] Only players can use this command!"));
        }
        return false;
    }
